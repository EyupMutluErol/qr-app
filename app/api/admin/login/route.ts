...
export async function POST(request: Request) {
  try {
    ...
    // bu kullanıcı adında admin var mı veritabanında kontrol ediyorum
    ...
    // şifrenin hashlenmiş hali veritabanındakiyle eşleşiyor mu kontrol ediyorum
    ...
    const token = await new SignJWT({ id: admin.id, username: admin.username, role: "admin" })
      ...
      .setExpirationTime('8h') // 8 saatlik bir token veriyorum
      .sign(JWT_SECRET);
